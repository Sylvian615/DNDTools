...
const CornugonSmash = {
  title: "角魔之猛击（Cornugon Smash ）【战斗】",
  source: "",
  description: `角魔之猛击（Cornugon Smash ）【战斗】
--------------------------------------------------------------------------------
先决条件：猛力攻击，威吓6级
专长效果：当你以猛力攻击的方式对你的对手造成伤害时，你可以立即以一个自由动作对你的对手做一次威吓检定用以挫败士气。
挫败士气 (Demoralize)：你可以尝试通过威吓技能使你的对手战栗一定的轮数。威吓检定DC为 {10+目标生物HD+感知修正}。如果你成功通过检定，目标就会战栗1轮，另外如果你的检定结果每高于DC 5点，战栗效果还能延长1轮。
你只能尝试挫败一个距离你30尺内、且可以听到和看见你的生物的士气。对同一个生物使用挫败士气只会延长持续时间；它不能造成更严重的恐惧效果。
`,
};
